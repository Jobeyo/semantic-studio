...
type Section = 'password' | 'users' | 'llm' | 'agent';
...
export default function SettingsPage() {
  ...
  const [agentConfig, setAgentConfig] = useState({ agentName: 'Studio AI', agentDescription: '', agentPersona: '', systemPromptExtra: '' });
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeSection === 'agent' && isAdmin && (
            <div className="space-y-6">
              <div>
                <h2 className="text-base font-semibold text-gray-900">AI-agent konfiguration</h2>
                <p className="text-sm text-gray-500 mt-1">Anpassa hur AI-assistenten beter sig i Studio.</p>
              </div>
              <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Agentens namn</label>
                  <input value={agentConfig.agentName} onChange={e => setAgentConfig(p => ({...p, agentName: e.target.value}))}
                ...
                <div>
                  ...
                  <input value={agentConfig.agentDescription} onChange={e => setAgentConfig(p => ({...p, agentDescription: e.target.value}))}
                    placeholder="Din AI-drivna modelleringsassistent"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Agentens roll och persona</label>
                  <textarea value={agentConfig.agentPersona} onChange={e => setAgentConfig(p => ({...p, agentPersona: e.target.value}))}
                    rows={4} placeholder="Du är en erfaren datamodellerare och BI-arkitekt..."
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" />
                  <p className="text-xs text-gray-400 mt-1">Beskriv agentens roll, expertis och kommunikationsstil.</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Extra instruktioner</label>
                  <textarea value={agentConfig.systemPromptExtra} onChange={e => setAgentConfig(p => ({...p, systemPromptExtra: e.target.value}))}
                ...
                <div className="flex justify-end">
                  <button onClick={async () => {
                    ...
                    {agentSaved ? '✓ Sparat!' : 'Spara konfiguration'}
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* Språk */}
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
        <h2 className="text-base font-semibold text-gray-900">Språk / Language</h2>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setLanguage('sv')}
            className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${language === 'sv' ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
            🇸🇪 Svenska
          </button>
          <button
            onClick={() => setLanguage('en')}
            className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${language === 'en' ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
            🇬🇧 English
          </button>
        </div>
        <p className="text-xs text-gray-400">{language === 'sv' ? 'Väljer visningsspråk i Studio' : 'Sets the display language in Studio'}</p>
      </div>
    </div>
  );
}
